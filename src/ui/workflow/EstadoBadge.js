...
export function renderEstadoBadgeHtml(data = {}, opts = {}) {
  ...
  return `<span class="sgc-estado-badge sgc-estado-badge--${catClass} badge-estado-mod${extra}" data-estado="${esc(entry.codigo)}" data-categoria="${esc(categoria)}" title="${esc(tooltip)}" aria-label="${esc(label)}"><span class="sgc-estado-badge__text">${esc(label)}</span></span>`;
}
