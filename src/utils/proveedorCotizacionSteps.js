...
import {
  ...
  cantidadPorTipo, normalizeTipoCotizacion, unidadMedidaCotizacion,
...
export function renderStep1Entregables(ctx, { locador = false } = {}) {
  ...
  const ecoRows = ents.map((e, eidx) => {
    const precio = Number(e.precio_unitario ?? e.precio ?? e.total ?? 0) || 0;
    const um = e.um || e.unidad_medida || 'Servicio';
    const nombreEnt = e.nombre || e.descripcion || `Entregable ${eidx + 1}`;
    const isFirst = eidx === 0;
    const rowspan = isFirst ? ` rowspan="${ents.length}"` : '';
    return `
      <tr data-eidx="0" data-enidx="${eidx}">
        ${isFirst ? `<td class="text-center align-middle"${rowspan}>1</td>
        <td class="small align-middle" style="white-space:normal;word-break:break-word;"${rowspan}>${esc(it0.descripcion || sol.denominacion || sol.objeto || '')}</td>` : ''}
        <td class="small" style="white-space:normal;word-break:break-word;">${esc(nombreEnt)}</td>
        <td class="text-center">${esc(um)}</td>
        <td><input class="form-control form-control-sm prov-e-unit text-end" type="text" inputmode="decimal"
          value="${esc(formatPriceDisplay(precio))}"${ro}></td>
        <td class="text-end small align-middle prov-e-total">${esc(money(precio))}</td>
      </tr>`;
  }).join('')
        ...
        <tbody>
          ${workspace.items.map((it, idx) => `
            <tr>
              ...
              <td class="text-center">${esc(unidadMedidaCotizacion(tipo, it.unidad_medida))}</td>
            </tr>`).join('')}
        </tbody>
      </table>
    </div>
      ...
      <table class="table table-bordered table-sm prov-cot-table mb-0">
        <thead class="table-light text-center align-middle">
          <tr>
            ...
            <th>Precio Unitario por cada entregable S/ (Inc. IGV)</th>
            <th>Precio Total S/ (Inc. IGV)</th>
          </tr>
        </thead>
        <tbody>${ecoRows}</tbody>
      </table>
...
export function initEntregablesEco(items, prevEco = {}, tipo, workspace = null) {
  ...
  items.forEach((it) => {
    ...
    map[it.item_key] = [{
      nro: 1,
      um: unidadMedidaCotizacion(t, it.unidad_medida),
      precio_unitario: 0,
      total: 0,
    }];
  });
