...
// codigo_pedido = identificador interno (no mostrado como pedido)
// =====================================================
import { createCrudView } from './crudViewFactory.js';

const FIELDS = [
  { name: 'ano_eje', label: 'Año Ejecución', col: 3, required: true },
  { name: 'tipo', label: 'Tipo', type: 'select', options: ['B', 'S', 'L'], col: 3, required: true },
...
const COLUMNS = [
  { name: 'pedido_sigamef', label: 'Pedido SIGAMEF', width: '110px' },
...
const view = createCrudView({
  ...
  icon: 'bi-card-list',
  subtitle: 'Gestión de Pedidos SIGAMEF — importación UPSERT, exportar Excel y CRUD',
  fields: FIELDS,
  columns: COLUMNS,
  excel: true,
