/**
 * Descubrimiento y limpieza del fixture RC8156G2 (test G-2).
 * Solo afecta órdenes RC8156G2*, usuarios g2_* / Fixture G2, y workflows huérfanos ligados a ellos.
 */
import { query } from '../../server/db.js';

const ORDEN_PATTERN = '^RC8156G2';
const USER_USERNAME_PATTERN = '^g2_';
const USER_NOMBRE_PATTERN = '^Fixture G2 ';

export async function discoverRc8156G2Fixture(runQuery = query) {
  ...
  const usuarios = (await runQuery(`
    SELECT id, username, nombre, dni, cargo, centro, activo
    FROM usuarios
    WHERE username ~ $1 OR nombre ~ $2
    ORDER BY id
  `, [USER_USERNAME_PATTERN, USER_NOMBRE_PATTERN])).rows;
  ...
  if (usuarioIds.length) {
    const workflowRefs = (await runQuery(`
      SELECT id, estado, motivo, usuario_origen_id, usuario_destino_id, expediente_id
      FROM workflow_observaciones
      WHERE usuario_origen_id = ANY($1::int[])
         OR usuario_destino_id = ANY($1::int[])
      ORDER BY id
    `, [usuarioIds])).rows;
    for (const row of workflowRefs) workflowIdSet.add(Number(row.id));
    workflows = workflowRefs;
  }

  if (workflowIdSet.size) {
...
export function hasRc8156G2Residuals(snapshot) {
  return snapshot.totals.usuarios > 0
    || snapshot.totals.ordenes > 0
    || snapshot.totals.workflows > 0
    || snapshot.totals.observaciones > 0
    || snapshot.totals.entregas > 0;
}

export async function cleanupRc8156G2Fixture({
  apply = false,
  tracked = null,
  runQuery = query,
  getClient = null,
} = {}) {
  const snapshot = await discoverRc8156G2Fixture(runQuery);
  if (!hasRc8156G2Residuals(snapshot)) {
...
export function printRc8156G2Snapshot(snapshot) {
  console.log('--- Candidatos RC8156G2 ---');
  ...
  for (const u of snapshot.usuarios) console.log(`    #${u.id} ${u.username} (${u.nombre}) activo=${u.activo}`);
  console.log(`  workflows: ${snapshot.totals.workflows}`);
  for (const w of snapshot.workflows) console.log(`    #${w.id} ${w.estado} orig=${w.usuario_origen_id} dest=${w.usuario_destino_id}`);
